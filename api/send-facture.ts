// Fonction serverless Vercel : génère une facture PDF et l'envoie au client par
// email (Resend, pièce jointe). Réservée aux admins authentifiés. Calquée sur
...
export default async function handler(req: any, res: any) {
  ...
  if (refErr || !ref) {
    res.status(500).json({ error: 'Numérotation de la facture impossible' })
    return
  }
