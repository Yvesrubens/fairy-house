...
// Émission d'une facture (Q2). Calqué sur DevisForm : lignes éditables, TVA 20 %,
// envoi au client via /api/send-facture (PDF « FACTURE » en pièce jointe).
...
export default function FactureForm({
  ...
  async function submit(e: FormEvent) {
    ...
    try {
      ...
      if (!res.ok) throw new Error(body.error || 'Échec de l’envoi de la facture')
      ...
      >
        <div className="flex items-center justify-between">
          <h2 className="text-xl font-bold text-gray-900">
            Générer une facture — {reservation.client_name}
          </h2>
          <button
            type="button"
            onClick={onCancel}
        ...
        <div className="mt-8 flex gap-3">
          ...
          >
            {busy ? 'Envoi…' : 'Générer et envoyer la facture'}
          </button>
          <button
            type="button"
            onClick={onCancel}
